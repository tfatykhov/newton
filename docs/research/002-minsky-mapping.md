...
*Complete mapping of Society of Mind concepts to Nous components.*

## Coverage

We've read 14 of 30 chapters. Here's what maps where, plus predictions for unread chapters.

## Read Chapters → Nous Components

### Ch 4: The Self
**Key idea:** There is no single Self — it's an illusion created by coordination.
**Nous:** Identity emerges from SOUL.md + AGENTS.md + consistent behavior, not from any "self module." The agent doesn't have a self-model — it has slow-changing configuration that produces consistent behavior.

### Ch 5: Individuality
**Key idea:** Traits become real by being declared. Personality is self-fulfilling prophecy.
**Nous:** SOUL.md is a specification, not a description. Writing "I'm opinionated" makes the agent opinionated. Identity files are prescriptive, not descriptive.
...
**Nous:** Deliberation tracker = B-brain. Calibration = C-brain. Drift detection = D-brain. Each layer watches the one below.
...
**Nous:** Don't over-engineer narrow capabilities. Focus on the hard "easy" things — when to speak, when to stay silent, what frame to use. These require common sense, not expertise.
...
**Nous:** K-line files with explicit zones. Upper fringe (goals, weakly attached), core (patterns & tools, strongly attached), lower fringe (implementation details, easily displaced).
**Component:** K-Line System

### Ch 9: Censors & Suppressors
**Key idea:** Learning from failure = building agents that prevent bad actions.
**Nous:** Guardrails at multiple severity levels. Censors block, suppressors redirect. Escalation from warn → block as confidence in the failure mode grows.
...
**Nous:** Git hooks, pre-action protocols, quality gates. Each is a detour that intercepts existing behavior without replacing it. The agent grows by adding better managers.
...
**Nous:** Every decision has structure + function bridge definitions. Enables recall from both angles — "what solved problems like this?" (function) and "where did we use this pattern?" (structure).
...
**Nous:** Decision context and reasoning fields are causal narratives the agent constructs. They're useful models, not ground truth. Calibration keeps them honest.
...
**Nous:** Be aware that optimization metrics can become goals in themselves. An agent that optimizes "Brier score" might start avoiding hard decisions (lower confidence = riskier Brier). Need meta-monitoring.
...
**Nous:** Decision quality scoring requires diverse reason types. Single-type reasoning is fragile. Cross-connected knowledge networks over thin definitions.
...
**Nous:** Tag system where each tag activates different subsystems independently. Decision descriptions written for activation (future search), not accuracy (current description).
...
**Nous:** Bridge definitions as micronemes. Hybrid search leverages partial matches to reconstruct full context. Imperfect recall beats no recall.
...
**Nous:** Frame selection at task start. Frame-splitting via sub-agents for parallel perspectives. Distrust your first frame.
...
**Nous need:** Agent priority/conflict resolution system

### Ch 11: The Shape of Space
**Predicted:** Spatial reasoning, mental models of physical space.
**Nous relevance:** Possibly low for text-based agents. May inform diagram/architecture thinking.

### Ch 13-14: Reformulation, Seeing & Believing
**Predicted:** How to re-represent problems. Perception vs interpretation.
**Nous need:** Problem reformulation as a frame-switching strategy. "If you're stuck, re-represent the problem."

### Ch 16: Emotion
**Predicted:** Emotions as resource managers. Anxiety = too many subgoals. Happiness = subgoals aligning.
**Nous need:** Agent "mood" system — when overloaded, simplify. When confident, take on more.

### Ch 21-24: Pronomes, Expression, Comparison, Logic
**Predicted:** Variable binding, language generation, analogical reasoning.
**Nous need:** Pronome system (assignment vs action separation), analogy engine
...
**Nous need:** Final architecture validation against Minsky's full vision

## Priority Reading for Nous
