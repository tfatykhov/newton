...
class CognitiveLayer:
    ...
    async def end_session(
        ...
        2. If reflection provided, extract facts:
           - Parse reflection for "learned: ..." lines
           - Store each as a fact via Heart.facts.learn() with source="reflection"
        3. Remove from self._active_episodes
        4. Emit "session_ended" event with reflection summary

        Note: The reflection text is generated by the Runtime (005) via
        an LLM call BEFORE calling end_session(). The Cognitive Layer
        itself makes no LLM calls — it just processes the reflection.
        The Runtime decides whether to spend the LLM call based on
        session length (skip for conversations < 3 turns).
        """
...
# test_end_session_with_reflection — reflection text stored as episode lessons
# test_end_session_reflection_extracts_facts — "learned: X" lines become facts
...
### D9: Markdown output for LLM consumption (from research/013)
All `_format_*` methods in ContextEngine render data as clean markdown with natural language, NOT as JSON or structured data. LLMs comprehend markdown files significantly better than serialized objects. Each section uses markdown headers, bullet lists, and inline formatting. This is a deliberate design choice backed by LangChain's production experience.

### D10: Reflection is generated externally, processed internally (from research/013)
The Cognitive Layer itself makes no LLM calls (D1). End-of-session reflection is generated by the Runtime (005) and passed to `end_session(reflection=...)`. The Cognitive Layer parses structured output ("learned: X" lines) into fact extraction calls. This keeps the cognitive layer deterministic while still enabling LLM-powered learning.

### D11: Contradiction surfacing over silent acceptance (from research/013)
When Heart.facts.learn() detects a potential contradiction (planned enhancement from 003 post-ship notes), the Cognitive Layer should surface it rather than ignore it. In `pre_turn`, if the recall phase finds contradicting facts, include both in context with a note: "⚠️ Contradicting facts found — resolve before proceeding." Let the LLM decide, don't hide the conflict.
